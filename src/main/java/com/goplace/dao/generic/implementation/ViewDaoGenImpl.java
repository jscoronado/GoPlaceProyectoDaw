 ...
 */
package com.goplace.dao.generic.implementation;
                    ...
                    if (!method.getName().substring(3).equalsIgnoreCase("id")) {
                        if (method.getName().substring(0, 3).equalsIgnoreCase("set")) {
                            ...
                            if (method.getName().length() >= 5) {
                                ...
                                //GenericDaoImplementation oAjenaDao = (GenericDaoImplementation) Class.forName("com.goplace.dao." + strAjena + "Dao").newInstance();

                                    //rafa: aqui intentamos crear el DAO de la clave ajena generico-específico y si no espcífico
                                    //pte porque da error
                                    Constructor c;
                                    try {
                                        c = Class.forName("com.goplace.dao.generic.specific.implementation." + strTabla + "DaoGenSpImpl").getConstructor(String.class, Connection.class);
                                    } catch (ClassNotFoundException | NoSuchMethodException | SecurityException ex) {
                                        //aqui da el error--> pte d eestudiar
                                        c = Class.forName("com.goplace.dao.specific.implementation." + strTabla + "DaoSpcImpl").getConstructor(String.class, Connection.class);
                                    }
                                    //------------------------------------

                                    TableDaoGenImpl oAjenaDao = (TableDaoGenImpl) c.newInstance(strTabla, connection);

                                    BeanGenImpl oAjenaBean = (BeanGenImpl) Class.forName("com.goplace.bean.generic.specific.implementation." + strTabla + "BeanGenSpImpl").newInstance();
